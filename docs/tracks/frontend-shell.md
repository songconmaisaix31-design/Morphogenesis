...
- 收尾复验（Chromium 1234，2026-09-23）：`npm run build`（`finals-shell.js` 221.00 kB、CSS 66.28 kB）及 `node --check viz/static/app.js` 均通过。实际 1366×768、768×768、375×768 视口中 `MORPHOGENESIS` 的边界都落在视口内、`scrollWidth === innerWidth`；截图为 `C:/Temp/morph-shell-desktop.png`、`C:/Temp/morph-shell-phone.png`、`C:/Temp/morph-shell-swarm-detail.png`。空仪表盘的 provenance 徽章显示“未加载导出”，不把 `empty_dashboard()` 的 `live` 值写成已加载 live 数据。
- EvoMap 点击链浏览器复验（同 Chromium；本地页面配 E 冻结响应契约 mock）：首次进入 swarm 仅一次 `/api/evomap`，渲染卡片时零次 `/api/evomap/asset`，点击一张 Gene 卡后恰好一次 `GET /api/evomap/asset?id=sha256:<64 hex>`；`asset_detail`、`asset_timeline`（事件）和 `gene_branches`（分支）均可见，反复 PHYSARUM/swarm 切换不产生额外请求，页面错误为 0。实际 E 服务代码在独立 E worktree，未在本 S worktree 启动；因此此项是浏览器契约验证，并非当次 live Hub 验收。
