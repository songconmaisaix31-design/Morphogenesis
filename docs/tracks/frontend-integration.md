...
- 按精确 SHA 普通 `--no-ff` 合并 P 修订：`2a501ada963c5b185c615ef79e727272e1093cfc` → `c843d95`；P 二次视觉修订 `190e597bbd91225f1b9bad27c0a25e7f873c74e5` → `b5b1c41`，均无冲突。首次修订在真实 Chromium 1366×768 中仍呈连续丝带/叶片，未通过视觉门槛，截图留在忽略的 `.runtime/physarum-check-20260923/`。
- 二次修订的 WebGL 图在首屏呈黄色、不规则、连通的分支纹脉；鼠标悬停时向食物点收拢，指针实际移出视口（`page.mouse.move(-20,-20)`）后恢复到中央网络。大标题前景下仍可辨认。截图：`.runtime/physarum-check-v2-20260923/01-idle.png`、`02-hover.png`、`03-recovery.png`、`05-reduced.png`。它仍是风格化模拟，不是生物显微录像。
