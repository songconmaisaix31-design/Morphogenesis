...
8. Validate using immutable operator-seeded task acceptance. Prepare A's target application and durably save the submission identity while renewal is active, outside a SQLite write transaction. Join the renewal thread, perform one fenced renewal with the same configured TTL, and immediately call B's `submit` with that exact latest lease and a short prepared callback. There is no intervening status-file write, Git, SDK, model call or test. The callback checks the submit-side fence immediately before each target write. Expiry during the final handoff or publication still rejects the effect.
...
Mirror receipt files are exclusively created per asset in the background **before** store/SDK preparation. The initial `PublicationRecord` has an empty, unsendable payload and reason `mirror_preparation_pending`; it describes local preparation, not a Hub request or acknowledgment. Store/SDK exceptions leave `unknown` with a fixed reason; known approval, lineage, provenance or local-approval rejection leaves `rejected`. Successful preparation reuses the direct adapter's atomic record writer, then its unchanged `publish`. A preparation update checks that its own marker is unchanged, and an adapter exception preserves any outcome already written. Existing pending/unknown/rejected/received records prevent repeat sends, including across restart.

The observer projects adapter states into `pending`, `confirmed`, `rejected`, `unknown`, retaining the original adapter state. It exposes only allowlisted reasons, sources, provenance and acceptance dimensions, excluding payload, arbitrary exception strings and original-run URI. `confirmed` means acknowledged transport, not Hub promotion; `hub_promoted` is separately true only for the adapter's `promoted` state. Observation never creates or updates these records.

`enqueue=True` means admitted to the bounded **memory** queue, not a durable receipt or a Hub send. Disabled/closed mirrors, invalid asset IDs, missing explicit approval, missing official proxy lineage, and a full queue return false without creating records. Queued unsent items can be lost when the process exits. If the directory/disk cannot persist a record, the mirror exposes the single process-local `last_error="mirror_record_unavailable"`; an existing pending/unknown record is retained where possible, but durable visibility cannot be guaranteed under storage failure. No exception text is retained and no automatic retry occurs. This optional mirror is not a durable delivery service, and its failures never change local outcomes.
...
## Submission handoff investigation, 2026-09-24

CI [35889933044](https://github.com/songconmaisaix31-design/Morphogenesis/actions/runs/35889933044), at `d4808870bf5b196babf40ebf3f82503365fe9074`, passed Ubuntu with **466 passed, 1 skipped in 207.09s** and all later gates. Windows reported **1 failed, 466 passed in 469.18s**: the 2-second wall-clock renewal test returned `stopped`. Its truncated assertion did not expose the reason. The original `.runtime/swarm-integration-20260924/ci-windows.log` is preserved; that CI failure and the earlier 0.25-second Linux failure are not retrospectively assigned a proven cause.

A separate deterministic scheduling counterexample exposed an actual submission gap. Before the fix, renewal stopped before `_status("submitting")`, whose `fsync/replace` could consume the remaining lease. Delaying only that preparation write for 2.5 seconds with a 2-second TTL failed in **11.75s**. The complete safe diagnostic showed nine successful renewals, maximum renewal call 0.047s, maximum scheduling gap 0.688s, preparation/SDK/Git phase 3.234s, status phase 2.516s, and `LeaseLost` at submit. Owner, token and expiry still matched, but the lease had expired by about 0.696s; `effect_applied` remained false. Evidence is retained in `.runtime/swarm-integration-20260924/c-status-gap-before.log` and the isolated `pytest-127` state. This confirms the injected gap, not the cause of the historical CI run.

Preparation IO now retains background renewal. The final handoff joins that thread and requests a single fresh lease using the existing TTL, after which only in-memory metadata preparation and B's short submission transactions remain. B still checks exact expiry/owner/token/status and live TTL. A still checks the fence before each file effect and rolls back known failures. No domain schema, SQLite transaction, SDK, Git helper or lease-fencing rule changed. Scheduling or publication stalls beyond the configured TTL still fail closed; this is not a real-time guarantee.

Failure status and audit now carry safe stage, error class, allowlisted reason, monotonic phase durations, renewal count/call/scheduling durations, time since renewal/stop, and the current task's token/expiry/owner/effect comparisons. Numeric OS/SQLite error codes are allowed; arbitrary exception strings, remote payloads and credentials are not. Diagnostic reads are limited to the current task and never affect routing. Renewal assertions use an untruncated JSON failure message so future CI logs retain these facts.

The injected gap, original 2-second real-clock test and controlled-clock protocol test passed together: **3 passed in 29.88s**, local Windows/Python 3.12, with BLAS threads set to 1. Full Worker regression then passed **25 tests in 149.46s**. After adding safe validation-rejection reasons, the two affected diagnostic tests plus concurrent local/API mock acceptance passed **20 tests in 205.49s**. These counts overlap and are separate runs. Final commands:

```powershell
# OPENBLAS_NUM_THREADS=OMP_NUM_THREADS=MKL_NUM_THREADS=1
./.venv/Scripts/python.exe -m pytest tests/swarm/test_worker_runtime.py -q
./.venv/Scripts/python.exe -m pytest tests/swarm/test_worker_runtime.py::test_failed_validation_retains_quarantine_and_accounts_usage tests/swarm/test_worker_runtime.py::test_failure_diagnostics_exclude_exception_text tests/swarm/test_selfgrowth.py tests/swarm/test_worker_evomap.py -q
./.venv/Scripts/python.exe -m mypy --strict swarm/worker_loop.py
./.venv/Scripts/python.exe -m mypy --strict --platform linux swarm/worker_loop.py
# Both strict checks: no issues found in 1 source file
```

The new counterexamples also prove that expiry after final handoff still rejects before target mutation, an expired/replaced holder cannot renew, a crash retaining the pre-handoff expiry recovers only completed authority without re-execution, and exception text is excluded. Logs `c-runtime-after.log`, `c-acceptance-after.log`, `c-mypy-windows.log` and `c-mypy-linux-target.log` are retained beside the original CI log. No TTL was enlarged and the real-clock test remains. Full repository/build/SDK/distribution gates and exact-commit Windows/Linux CI belong to independent I; Linux-target static checking here is not actual Linux execution. Historical CI causes remain unknown, and all repair evidence remains `contract_local`; no real API, key read, Hub publication, mainline merge or deployment was performed.

## Mirror visibility repair, 2026-09-24

Independent I's `.runtime/swarm-integration-20260924/mirror-gap-before.json` showed `enqueue=True`, zero network calls and an empty/missing observer after `store.state` raised before record creation. C reproduced this against the unchanged mirror code at `39a8182`: **1 failed in 16.46s**, retained as `c-mirror-gap-before.log`. The repaired first gate passed **10 tests in 35.71s**. An expanded check then reported **1 failed, 22 passed in 42.75s** (`c-mirror-final.log`): the test incorrectly expected a nonexistent asset's throwing state lookup to mean known quarantine. That lookup remains `unknown/preparation_failed`; a separate actual candidate published without validation/promotion verifies known quarantine as `rejected`. No broad exception reclassification was introduced to make the test pass.

Final mirror/observer gate: **27 passed in 52.68s**, local Windows/Python 3.12, with `OPENBLAS_NUM_THREADS`, `OMP_NUM_THREADS` and `MKL_NUM_THREADS` set to 1. Commands:

```powershell
./.venv/Scripts/python.exe -m pytest tests/swarm/test_mirror.py tests/swarm/test_observer.py -q
./.venv/Scripts/python.exe -m mypy --strict swarm/hub_mirror.py swarm/observer.py
./.venv/Scripts/python.exe -m mypy --strict --platform linux swarm/hub_mirror.py swarm/observer.py
# Both strict checks: no issues found in 2 source files
```

The gate covers visible pending before store access; store/SDK errors; actual quarantined candidates; approval/source/provenance rejection; no resends or receipt changes across restart; preserving confirmed/unknown outcomes during preparation or adapter failure; offline bounded enqueue and queue capacity; disabled/closed/invalid/missing-approval offers; unwritable storage; and readonly, allowlisted observer projection including malformed records. Evidence remains in `c-mirror-regression.log`, `c-mirror-strict-windows.log` and `c-mirror-strict-linux-target.log` beside I's original JSON. Only the five assigned mirror/observer/test/runtime-document files changed; SDK, A/B domains and Worker renewal are untouched. All transport evidence uses local `MockTransport`/fixtures and remains `contract_local`; no real Hub request, credential lookup, service startup, deployment or mainline change occurred. Independent I retains ownership of full-repository and exact-SHA cross-platform gates.
