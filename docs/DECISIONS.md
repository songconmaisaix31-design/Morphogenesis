...
现场阶段（2026-09-22）：演示执行器确定为 EvoMap 网关，OpenCode 是按难度分档的并行开发备选，今晚不验证其工具调用/流式。用户要求继续多 Agent 后，只拆现场展示与人工证据两条互斥轨；观察器增加显式人工 Enter 模式，不改变已验收的默认自动模式或第四轮原始证据。实接投影和彩排必须现场见证；Hub 缺沙箱保持本地 stub 并标“待发布”；不实现或宣传在途进程强杀恢复，统一表述为“成员下线后，后续任务自动重新选路”。
