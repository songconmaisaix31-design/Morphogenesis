...
M7 observer.py 只读账本、租约、审计、ValidationReport。hub_mirror.py 复用 E 直连实现，默认关闭、异步可选且四态可见。本轮不调用生产 Hub，不搜索秘密。模型调用按下节用户追加要求执行。

## 2026-09-24 用户追加：EvoMap API 算法尝试

所有参与算法尝试的测试 Agent 和蜂群 Agent 统一使用 EvoMap 提供的模型 API。复用现有网关 `https://api.evomap.ai/v1` 与已验证模型配置；默认沿用 `evomap-gpt-5.6-luna`，不把 Hub 资产查询、官方 SDK 或本地 fixture 当作模型执行。此要求授权本轮有界模型实验，替代此前本轮不新增模型调用的限制；冻结主线、Hub 发布、预算和任意代码执行边界不变。

运行轨提供显式 EvoMap 执行入口和无秘密的实验配置；确定性并发、断网、故障注入回归仍保留本地证据类别。真实实验通过共享 SQLite 预算预留，限定总请求数、每任务请求数、输出长度、运行时长和准入预算；不自动重试、换根重发或故障后改用另一个模型。无可信单请求费用上界则明确 `request_bound=unbounded`，原预留不因估算较低释放；未知 usage/远端效果停止新请求。真实价格缺失时不能把测试价格当作提供商计价。

凭据只来自明确命名的环境变量或用户指定的凭据路径，不能进入 argv、模型上下文、审计、Git、observer、验证器或 SDK 子进程。记录请求与返回模型、请求标识、HTTP 状态、真实 usage、耗时、路由输入和概率、验证报告及实际采用链；费用未对账时保持未知。远端输出仅形成受限数据候选，由独立固定策略验证，不放开任意代码执行。

验收分两层：API 真实回执证明 `interface_live`；三进程自主选择、任务结果通过验证和跨成员真实采用才支持对应 `task_live` 场景。凭据或可信计价配置缺失时继续完成接入与离线门禁，真实实验保留待运行，不能伪造通过。当前已核对 Process/User/Machine 均未设置 `MORPH_EVOMAP_API_KEY`；已请求用户提供安全配置位置。
