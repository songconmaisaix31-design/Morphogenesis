...
(async () => {
  ...
  try {
    for (const [width, height] of [[1366, 768], [1920, 1080], [375, 812]]) {
      ...
      check(`${width} only active panel visible`, await page.locator('[role=tabpanel]:visible').count() === 1 && await page.locator('#backend-overview').isVisible());
      check(`${width} source application frame`, await page.locator('.Mmx1Wq_sidebar .Mmx1Wq_switchWorkspaceButton').count() === 1 && await page.locator('.WS84WW_view > .KFZpfa_panel > ._1uFtza_locationBar').count() === 1);
      check(`${width} content toolbar 44px`, await page.locator('.backend-topbar').evaluate(el => el.getBoundingClientRect().height === 44));
      if(width > 720) check(`${width} toolbar starts after sidebar`, await page.locator('.backend-topbar').evaluate(el => el.getBoundingClientRect().left === 241));
      check(`${width} metrics are compact properties`, await page.locator('.backend-metrics .KFZpfa_propertyRow').evaluateAll(rows => rows.length === 3 && rows.every(el => el.getBoundingClientRect().height <= 26)));
      check(`${width} technical states are in details`, !(await page.locator('#contract_local').isVisible()));
      await page.screenshot({ path: path.join(output, `workspace-${width}.png`) });
      await page.getByRole('button', { name: '验收详情', exact: true }).click();
      check(`${width} complete acceptance is accessible`, await page.locator('#contract_local').isVisible() && await page.locator('#interface_live').textContent() === 'not_run' && await page.locator('#task_live').textContent() === 'not_run');
      check(`${width} details text readable`, await page.locator('#backend-details .state-row code').first().evaluate(el => parseFloat(getComputedStyle(el).fontSize) >= 12));
      await page.screenshot({ path: path.join(output, `details-${width}.png`) });
      await page.keyboard.press('Escape');
      check(`${width} details Escape restores focus`, await page.getByRole('button', { name: '验收详情', exact: true }).evaluate(el => el === document.activeElement));
      ...
      await page.screenshot({ path: path.join(output, `empty-topology-${width}.png`) });
      await page.getByRole('tab', { name: 'Gene 池', exact: true }).click();
      check(`${width} Gene view has a sized chart`, await page.waitForFunction(() => { const el=document.querySelector('#gene-chart'); return window.echarts?.getInstanceByDom(el)?.getWidth() > 200; }).then(() => true));
      ...
      check(`${width} Envelope feed labels its mock source`, await page.locator('#event-feed').textContent().then(text => text.includes('Envelope') && text.includes('mock') && text.includes('builder#0')));
      check(`${width} unknown tokens and lifecycle count preserved`, await page.locator('#metric-tokens').textContent() === '未知' && await page.locator('#story-gene-count').textContent() === '未知');
      check(`${width} acceptance does not upgrade`, await page.locator('#interface_live').textContent() === 'not_run' && await page.locator('#task_live').textContent() === 'not_run');
      ...
      await page.screenshot({ path: path.join(output, `genes-${width}.png`) });
      await page.keyboard.press('ArrowDown');
      check(`${width} arrows switch actual panels`, await page.locator('#backend-evidence').isVisible() && !(await page.locator('#backend-genes').isVisible()));
      check(`${width} hidden Gene chart disposed`, await page.waitForFunction(() => !window.echarts.getInstanceByDom(document.querySelector('#gene-chart'))).then(() => true));
      check(`${width} message chart resized after switch`, await page.waitForFunction(() => window.echarts.getInstanceByDom(document.querySelector('#message-chart'))?.getWidth() > 200).then(() => true));
      await page.keyboard.press('Tab');
      check(`${width} Tab skips hidden panel controls`, await page.evaluate(() => !document.activeElement.closest('[hidden],[inert]')));
      await page.screenshot({ path: path.join(output, `evidence-${width}.png`) });
      await page.getByRole('tab', { name: 'EvoMap 只读', exact: true }).click();
      check(`${width} navigation changes active section`, await page.locator('#backend-evomap').isVisible() && await page.locator('[role=tabpanel]:visible').count() === 1);
      ...
      check(`${width} failure preserves exported facts`, await page.locator('#story-genes').textContent().then(text => text.includes('review-first')));
      await page.getByRole('tab', { name: 'Gene 池', exact: true }).click();
      check(`${width} switching panels preserves 503`, await page.locator('#connection-state').innerText().then(text => text.includes('保留上次快照')) && await page.locator('#story-genes').isVisible());
    ...
    if (replayPath) {
      ...
      for (const [width, height] of [[1366, 768], [1920, 1080], [375, 812]]) {
        ...
        check(`${width} replay provenance remains downgraded`, await page.locator('#provenance').innerText().then(text => text.includes('replay')) && await page.locator('#task_live').textContent() === 'not_run');
...
if(width === 375) check('phone member controls have 36px target', await page.locator('.swarm-member-list button').evaluateAll(rows => rows.every(el => el.getBoundingClientRect().height >= 36)));
        ...
        check(`${width} member detail is reachable in content viewport`, await page.locator('.swarm-detail').evaluate(el => { const r=el.getBoundingClientRect(), frame=document.querySelector('.backend-content-scroll').getBoundingClientRect(); return r.top >= frame.top && r.bottom <= frame.bottom; }));
        ...
        await page.evaluate(() => document.querySelector('.backend-content-scroll').scrollTo(0, 0));
        ...
        check(`${width} replay task properties kept`, await page.locator('#metric-tokens').innerText() === '1348');
        check(`${width} replay system events remain compact`, await page.locator('.event-system').evaluateAll(rows => rows.length > 0 && rows.every(el => el.getBoundingClientRect().height <= 40)));
        await page.screenshot({ path: path.join(output, `replay-task-${width}.png`) });
        await page.close();
      }
    }
