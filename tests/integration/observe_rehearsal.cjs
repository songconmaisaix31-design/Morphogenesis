...
// Usage: node tests/integration/observe_rehearsal.cjs manual|auto PORT OUTPUT_DIR
...
const [mode, portText, outputArg] = process.argv.slice(2);
...
(async () => {
  ...
  const child = spawn('pwsh', ['-NoProfile','-File','demo/run-demo.ps1','-AuthorizeLive','-Mode',mode,'-Port',portText,'-Model','gpt-5.6-luna','-TimeoutSeconds','180','-TauSeconds','10','-StageDelay','2'], {windowsHide:true,stdio:['pipe','pipe','pipe']});
  ...
  try {
    ...
    fs.writeFileSync(path.join(output,'summary.json'),JSON.stringify({startedAt,endedAt:new Date().toISOString(),mode,root,port:Number(portText),parentPid:child.pid,exitCode,entered,failure,browser:browser.version()},null,2));
    await browser.close();
  }
